...
def plot_square_coil(start_x, start_y, initial_side, turns, spacing):
    ...
    for turn in range(turns):
        # Right
        for i in range(int(current_side / spacing)):
...
def generate_coil_json(start_x, start_y, initial_side, turns, spacing, track_width=0.15, filename=None):
    """
    ...
    if filename is None:
        filename = f"square_w{track_width:.2f}_s{spacing:.2f}_t{turns}.json"
